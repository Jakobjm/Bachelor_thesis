...
religion_data$pol_leaning[religion_data$pol_leaning == 0] <- "Atheist"
religion_data$pol_leaning[religion_data$pol_leaning == 1] <- "Religious"
...
classify_belief2 <-  function(input) {
	test <- unlist(tokenize_words(input,
																lowercase = TRUE, 
																strip_punct = TRUE, 
																strip_numeric = TRUE,
...
predict_naivebayes2 <- function(input_df) {
	predictions <- matrix(nrow = length(input_df$clean_text), ncol = 2)
	colnames(predictions) <- c("Atheism" ,"Religion")

	for (i in 1:nrow(input_df)) {
		text <- religion_data$clean_text[i]
		predictions[i,] <- classify_belief2(text)
	}
	
	name <- deparse(substitute(input_df))
	name <- paste0(name,"_predictions_dist") 
	assign(name, cbind(input_df, predictions),envir = .GlobalEnv	)
}

predict_naivebayes(religion_data)

predict_naivebayes2(religion_data)

ggplot(religion_data_predictions_dist, aes(x=Atheism-Religion))+
	geom_density()

sum(religion_data_predictions$pol_leaning == religion_data_predictions$predictions)/nrow(religion_data_predictions)

religion_data_predictions_ath <- religion_data_predictions %>% filter(pol_leaning == "Atheist")
religion_data_predictions_rel <- religion_data_predictions %>% filter(pol_leaning == "Religious")

sum(religion_data_predictions_ath$pol_leaning == religion_data_predictions_ath$predictions)/nrow(religion_data_predictions_ath)
sum(religion_data_predictions_rel$pol_leaning == religion_data_predictions_rel$predictions)/nrow(religion_data_predictions_rel)

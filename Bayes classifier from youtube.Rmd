...
# Preprocess etc.:
```{r}
guns_data <- read_csv("clean_proantiguns_6months.csv")

#Cleaning: 

sub_gun <- guns_data %>% 
	filter(pol_leaning == "pro") %>% 
	sample_n(size = nrow(filter(guns_data, pol_leaning == "anti")),prob = NULL, replace =FALSE)

guns_data1 <- guns_data %>% 
	filter(pol_leaning =="anti") %>% 
	rbind(sub_gun)


#creating an 80% train 20% test split
sample <- sample.int(n = nrow(guns_data1), size = floor(.80*nrow(guns_data1)), replace = F)
guns_train <- guns_data1[sample, ]
guns_test  <- guns_data1[-sample, ]


#tokenizing
guns_train$clean_text <- (tokenize_words(guns_train$clean_text,
																						lowercase = TRUE,
																						strip_punct = TRUE,
																						strip_numeric = TRUE,
...
table(guns_train$pol_leaning)

#prior probability
prior_anti <- (table(guns_train$pol_leaning) / sum(table(guns_train$pol_leaning)))[1]
prior_pro <- (table(guns_train$pol_leaning) / sum(table(guns_train$pol_leaning)))[2]
...
#seperate data
anti_data <- subset(guns_train, pol_leaning == "anti")
pro_data <- subset(guns_train, pol_leaning == "pro")
...
classify_belief <-  function(input) {
	...
	return(c(anti_pred, pro_pred,	if (pro_pred < anti_pred) {
		("pro")
	} else {("anti")}))
}
...
predict_naivebayes(guns_test)

#turning numeric predictions back to numeric
guns_test_predictions$anti <- as.numeric(guns_test_predictions$anti)
guns_test_predictions$pro <- as.numeric(guns_test_predictions$pro)
...
ggplot(guns_test_predictions, aes(x=anti-pro))+
	geom_density()
...
sum(guns_test_predictions$pol_leaning == guns_test_predictions$prediction)/nrow(guns_test_predictions)


#subsetting to see if prediction accuracy is different across leanings
guns_test_predictions_anti <- guns_test_predictions %>% filter(pol_leaning == "anti")
guns_test_predictions_pro <- guns_test_predictions %>% filter(pol_leaning == "pro")

#calculating prediction accuracy for cases of specific leaning
sum(guns_test_predictions_anti$pol_leaning == guns_test_predictions_anti$prediction)/nrow(guns_test_predictions_anti)
sum(guns_test_predictions_pro$pol_leaning == guns_test_predictions_pro$prediction)/nrow(guns_test_predictions_pro)


```


# summarising by author to get labels for nodes
```{r}
guns_test_predictions_to_nodes <- guns_test_predictions %>%
	select(author, anti, pro) %>% 
	group_by(author) %>% 
	summarise(mean_anti = mean(anti), mean_pro = mean(pro)) %>% 
	mutate(leaning = if_else(mean_anti>mean_pro,"anti", "pro"))
